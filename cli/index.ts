...
const checkConfig = () => {
  ...
  msg += 'Please launch Terfy and retry.';
  console.error(msg);
  process.exit(1);
};
...
const lsRemote = (pattern?: string) => {
  ...
  const URL = `https://api.npms.io/v2/search?q=${
    (pattern && `${pattern}+`) || ''
  }keywords:terfy-plugin,terfy-theme&size=250`;
  return got(URL)
    ...
    commandPromise = lsRemote(query)
      .then((entries) => {
        if (entries.length === 0) {
          ...
          console.error(`${chalk.red('Try')} ${chalk.green('terfy ls-remote')}`);
          process.exit(1);
        } else {
...
args.command('<default>', 'Launch Terfy');
...
const main = (argv: string[]) => {
  ...
  const env = Object.assign({}, process.env, {
    // this will signal Terfy that it was spawned from this module
    TERFY_CLI: '1',
  ...
  if (!flags.verbose) {
    ...
    if (process.platform === 'darwin') {
      //Use `open` to prevent multiple Terfy process
      const cmd = `open -b co.zeit.terfy ${args_}`;
      const opts = {
        env
      };

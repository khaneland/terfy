...
const applicationDirectory =
  process.env.XDG_CONFIG_HOME !== undefined
    ? path.join(process.env.XDG_CONFIG_HOME, 'terfy')
    : process.platform == 'win32'
    ? path.join(process.env.APPDATA!, 'Terfy')
    : os.homedir();

const devConfigFileName = path.join(__dirname, `../.terfy.js`);

const fileName =
  process.env.NODE_ENV !== 'production' && fs.existsSync(devConfigFileName)
    ? devConfigFileName
    : path.join(applicationDirectory, '.terfy.js');

/**
 * We need to make sure the file reading and parsing is lazy so that failure to
 * statically analyze the terfy configuration isn't fatal for all kinds of

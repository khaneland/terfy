...
function updatePlugins({force = false} = {}) {
  ...
  install((err: any) => {
    ...
    if (err) {
      ...
      const changed = cache.get('terfy.plugin-versions') !== pluginVersions && loaded === total;
      cache.set('terfy.plugin-versions', pluginVersions);
...
// to prevent slowness
if (cache.get('terfy.plugins') !== id || process.env.TERFY_FORCE_UPDATE) {
...
function syncPackageJSON() {
  ...
  const pkg = {
    name: 'terfy-plugins',
    description: 'Auto-generated from `~/.terfy.js`!',
    private: true,
    version: '0.0.1',
    repository: 'vercel/terfy',
    license: 'MIT',
    homepage: 'https://terfy.is',
    dependencies
  };
...
function requirePlugins(): any[] {
  ...
  const load = (path_: string) => {
    ...
    try {
      ...
      if (!exposed) {
        notify('Plugin error!', `${`Plugin "${basename(path_)}" does not expose any `}Terfy extension API methods`);
        return;
      }

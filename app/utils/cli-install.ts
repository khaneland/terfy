...
const addSymlink = () => {
  return checkInstall().then((isInstalled) => {
    if (isInstalled) {
      console.log('Terfy CLI already in PATH');
      return Promise.resolve();
    }
    console.log('Linking TerfyCLI');
...
const addBinToUserPath = () => {
  return new Promise((resolve, reject) => {
    try {
      ...
      // C:\Users\<user>\AppData\Local\terfy\app-<version>\resources\bin
      const binPath = path.dirname(cliScriptPath);
      // C:\Users\<user>\AppData\Local\terfy
      ...
      console.log('Adding TerfyCLI path (registry)');
...
export const installCLI = (withNotification: boolean) => {
  if (process.platform === 'win32') {
    addBinToUserPath()
      ...
      .catch((err) =>
        logNotify(withNotification, 'Terfy CLI installation failed', `Failed to add Terfy CLI path to user PATH ${err}`)
      );
  } else if (process.platform === 'darwin') {
    addSymlink()
      .then(() => logNotify(withNotification, 'Terfy CLI installed', `Symlink created at ${cliLinkPath}`))
      .catch((err) => {
        ...
        console.error(err);
        logNotify(withNotification, 'Terfy CLI installation failed', error);
      });
  } else {
    withNotification &&
      notify('Terfy CLI installation', 'Command is added in PATH only at package installation. Please reinstall.');
  }
};

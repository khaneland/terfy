// Print diagnostic information for a few arguments instead of running Terfy.
if (['--help', '-v', '--version'].includes(process.argv[1])) {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const {version} = require('./package');
  const configLocation = process.platform === 'win32' ? `${process.env.userprofile}\\.terfy.js` : '~/.terfy.js';
  console.log(`Terfy version ${version}`);
  console.log('Terfy does not accept any command line arguments. Please modify the config file instead.');
  console.log(`Terfy configuration file located at: ${configLocation}`);
  process.exit();
}
    ...
    .then(() => {
      ...
      if (!isDev) {
        ...
        if (config.getConfig().defaultSSHApp && !app.isDefaultProtocolClient('ssh')) {
          console.log('Setting Terfy as default client for ssh:// protocol');
          app.setAsDefaultProtocolClient('ssh');
        } else if (!config.getConfig().defaultSSHApp && app.isDefaultProtocolClient('ssh')) {
          console.log('Removing Terfy from default client for ssh:// protocol');

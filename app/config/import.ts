...
// Migrate Terfy2 config to Terfy3 but only if the user hasn't manually
// touched the new config and if the old config is not a symlink
const migrateTerfy2Config = () => {
  if (cfgPath === legacyCfgPath) {
    // No need to migrate
    return;
  }
  if (!existsSync(legacyCfgPath)) {
    // Already migrated or user never used Terfy 2
    return;
  }
  ...
  if (existsNew) {
    ...
    if (hasNewConfigBeenTouched) {
      ...
      notify(
        'Terfy 3',
        `Settings location has changed to ${cfgPath}.\nWe've backed up your old Terfy config to ${backupPath}`
      );
      return;
    }
  }

...
const applicationDirectory =
  process.env.XDG_CONFIG_HOME !== undefined
    ? join(process.env.XDG_CONFIG_HOME, 'terfy')
...
const legacyCfgPath = join(homeDirectory, cfgFile); // Terfy 2 config location
...
const plugins = resolve(cfgDir, '.terfy');
const plugs = {
  legacyBase: resolve(homeDirectory, '.terfy_plugins'),
  legacyLocal: resolve(homeDirectory, '.terfy_plugins', 'local'),
  base: plugins,
  local: resolve(plugins, 'local'),
  cache: resolve(plugins, 'cache')
};
const yarn = resolve(__dirname, '../../bin/yarn-standalone.js');
const cliScriptPath = resolve(__dirname, '../../bin/terfy');
const cliLinkPath = '/usr/local/bin/terfy';

...
import {TerfyState, TerfyDispatch, TerfyActions} from '../terfy';
import {Stats} from 'fs';

const {stat} = window.require('fs');

export function openContextMenu(uid: string, selection: any) {
  return (dispatch: TerfyDispatch, getState: () => TerfyState) => {
    dispatch({
...
export function resetFontSize(): TerfyActions {
  return {
...
export function windowGeometryUpdated(): TerfyActions {
  return {
...
const findChildSessions = (termGroups: TerfyState['termGroups']['termGroups'], uid: string): string[] => {
...
const getGroupUids = (state: TerfyState) => {
...
export function enterFullScreen(): TerfyActions {
  return {
    type: UI_ENTER_FULLSCREEN
  };
}

export function leaveFullScreen(): TerfyActions {
  return {
...
export function execCommand(command: string, fn: (e: any, dispatch: TerfyDispatch) => void, e: any) {
  return (dispatch: TerfyDispatch) =>
    dispatch({

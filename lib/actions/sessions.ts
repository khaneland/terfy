...
import {TerfyState, session, TerfyDispatch, TerfyActions} from '../terfy';
...
function createExitAction(type: typeof SESSION_USER_EXIT | typeof SESSION_PTY_EXIT) {
  return (uid: string) => (dispatch: TerfyDispatch, getState: () => TerfyState) => {
    return dispatch({
      type,
      uid,
      effect() {
...
export function clearActiveSession(): TerfyActions {
  return {
    type: SESSION_CLEAR_ACTIVE
  };
}

export function setSessionXtermTitle(uid: string, title: string): TerfyActions {
  return {

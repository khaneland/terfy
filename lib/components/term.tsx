...
export default class Term extends React.PureComponent<TermProps> {
    ...
    if (!props.term) {
      ...
      if (props.webGLRenderer) {
        if (needTransparency) {
          ...
          // Experimental WebGL renderer needs some more glue-code to make it work on Terfy.
  ...
  getTermDocument() {
    console.warn(
      'The underlying terminal engine of Terfy no longer ' +

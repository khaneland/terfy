...
// https://github.com/vercel/terfy/issues/619
...
Module._load = function _load(path: string) {
  ...
  switch (path) {
    ...
    case 'react-dom':
      ...
      return ReactDOM;
    case 'terfy/component':
      console.warn(
        'DEPRECATED: If your plugin requires `terfy/component`, it must requires `react.PureComponent` instead and bundle `react` as a dependency'
      );
      return PureComponent;
    case 'terfy/notify':
      return notify;
    case 'terfy/Notification':
      return Notification;
    case 'terfy/decorate':
      return decorate;
    default:
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map((path: any) => {
      ...
      // mapTerfyTermState mapping for backwards compatibility with terfyterm
      if (mod.mapTerfyTermState) {
        mod.mapTerfyState = mod.mapTerfyTermState;
        console.error('mapTerfyTermState is deprecated. Use mapTerfyState instead.');
      }

      // mapTerfyTermDispatch mapping for backwards compatibility with terfyterm
      if (mod.mapTerfyTermDispatch) {
        mod.mapTerfyDispatch = mod.mapTerfyTermDispatch;
        console.error('mapTerfyTermDispatch is deprecated. Use mapTerfyDispatch instead.');
      }
      ...
      if (mod.mapTerfyState) {
        connectors.Terfy.state.push(mod.mapTerfyState);
      }

      if (mod.mapTerfyDispatch) {
        connectors.Terfy.dispatch.push(mod.mapTerfyDispatch);
      }
...
export function connect<stateProps, dispatchProps>(
  stateFn: (state: TerfyState) => stateProps,
  dispatchFn: (dispatch: TerfyDispatch) => dispatchProps,
  c: any,
  d: Options = {}
) {
  return (Class: any, name: keyof typeof connectors) => {
    return reduxConnect<stateProps, dispatchProps, any, TerfyState>(
      (state) => {
